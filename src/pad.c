...
{
    return (EXBaseGamePad__m_pGamePad->m_ButtonState[pad_num] & button) != 0;
}

inline bool pad_last_button_state(uint button, int pad_num)
{
    return (EXBaseGamePad__m_pGamePad->m_LastButtonState[pad_num] & button) != 0;
}

inline bool pad_button_edge_down(uint button, int pad_num)
{
    return (EXBaseGamePad__m_pGamePad->m_ButtonEdgeDown[pad_num] & button) != 0;
}

inline bool pad_button_edge_up(uint button, int pad_num)
{
    return (EXBaseGamePad__m_pGamePad->m_ButtonEdgeUp[pad_num] & button) != 0;
}
...
bool pad_button_edge_down_multiple(uint button, int num_presses, int time_limit, int pad_num)
{
    ...
    #else
    int index = (PAD_NUM_BUTTONS * pad_num);
    uint _button_acc = button;
    while (_button_acc > 1) {
        _button_acc >>= 1;
        index++;
    }
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][0];
}

inline float pad_analog_get_l_stick_y(int pad_num)
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][1];
}
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[g_PadNum][0];
}

inline float g_pad_analog_get_l_stick_y()
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[g_PadNum][1];
}
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][2];
}

inline float pad_analog_get_r_stick_y(int pad_num)
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][3];
}
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[g_PadNum][2];
}

inline float g_pad_analog_get_r_stick_y()
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[g_PadNum][3];
}
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][12];
}

float pad_analog_get_r_trigger(int pad_num)
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[pad_num][14];
}
...
{
    return EXBaseGamePad__m_pGamePad->m_AnalogChan[g_PadNum][12];
}
